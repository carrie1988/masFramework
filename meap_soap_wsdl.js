  ...
  if (child.name === 'include' || child.name === 'import') {
    ...
    if (location) {
      this.includes.push({
        ...
        this.$targetNamespace = this.$targetNamespace?this.$targetNamespace:(child.$namespace || child.$targetNamespace || this.$targetNamespace);
  }
...
DefinitionsElement.prototype.addChild = function(child) {
  ...
  else if (child.name === 'include' || child.name === 'import') {
        var location = child.$schemaLocation || child.$location;
        if (location) {
            this.includes.push({
                namespace: child.$namespace || child.$targetNamespace || this.$targetNamespace,
...
WSDL.prototype._xmlnsMap = function() {
  ...
  for (var alias in xmlns) {
    ...
    if (~ns.indexOf('http://xml.apache.org/'))
      continue;
    if(alias=='tns' && ns.indexOf('http')>-1){
      alias = ns.replace(/(.*?):\/\/(.*?)\.(.*)/,"$2");
