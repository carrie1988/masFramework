...
try {
    require("meap_redis_adapter");
    parsers.push(require("meap_redis_adapter"));
} catch (err) {
    ...
    self.send_command("auth", [this.auth_pass], function (err, res) {
        if (err) {
            if (err.toString().match("LOADING")) {
				...
				console.log("[MAS][MEAP_REDIS][ERROR]Auth error: " + err.message);
                return self.emit("error", new Error("Auth error: " + err.message));
            }
        }
        if (res.toString() !== "OK") {
			console.log("[MAS][MEAP_REDIS][ERROR]Auth failed: " + res.toString());
...
RedisClient.prototype.on_info_cmd = function (err, res) {
    ...
    if (err) {
		console.log("[MAS][MEAP_REDIS][ERROR]Ready check failed: " + err.message);
...
exports.print = function (err, reply) {
    if (err) {
        console.log("[meap_redis][print][ERROR]:" + err);
    } else {
        console.log("[meap_redis][print]:Reply - " + reply);
    }
};
